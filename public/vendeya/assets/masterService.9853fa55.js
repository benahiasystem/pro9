...
import { u as useUserSession, p as provideApi } from "./index.42a48c3f.js";
import { d as defineStore, u as useStorage } from "./vendor.4fd38f72.js";
...
const mapItem = (element) => {
  ...
  const item = {
    ...
    parentItemId: (_c = element.parent_item_id) != null ? _c : null,
    variationLabel: (_d = element.variation_label) != null ? _d : null,
    variationAttributes: (_e = element.variation_attributes) != null ? _e : [],
    variations: ((_f = element.variations) != null ? _f : []).map(mapItem)
  };
  return item;
};
const saveDataProducts = async () => {
  const { data } = await provideApi().get("/sellnow/items", { params: { group_variations: 1 } });
  const productsStorage = data.data.map(mapItem);
...
const saveDataConfiguration = async () => {
  ...
  if (configurations.printer_name_documents != null && !userSession.printerNameDocumentPinned) {
...
const searchCustomers = async (input, identityDocumentTypeId = null) => {
  ...
  return (_a = data.customers) != null ? _a : [];
};
const MasterService = {
  syncData,
  saveDataProducts,
