...
class Cash extends ModelTenant
{
    ...
    /** Estados históricos de comprobante que suman a la caja. */
    ...
    /**
     * Importe en bolívares: si la moneda no es VES se aplica el tipo de cambio.
     *
     * @param  string $amount
     ...
     */
    private function amountInNationalCurrency($amount, $currency, $rate)
    {
        return $this->getConnection()->raw(
            "COALESCE(SUM(CASE WHEN {$currency} = 'VES' THEN {$amount} ELSE {$amount} * {$rate} END), 0)"
        );
    }
    ...
    private function sumSaleNotePayments(array $valid_states)
    {
        ...
        $row = $this->currentTotalsBase()
            ...
            ->selectRaw($this->amountInNationalCurrency('snp.payment', 'sn.currency_type_id', 'sn.exchange_rate_sale').' as total')
            ->first();
    ...
    private function sumDocumentPayments(array $valid_states)
    {
        ...
        $row = $this->currentTotalsBase()
            ...
            ->selectRaw($this->amountInNationalCurrency('dp.payment', 'd.currency_type_id', 'd.exchange_rate_sale').' as total')
            ->first();
    ...
    private function sumExpensePayments()
    {
        $row = $this->currentTotalsBase()
            ...
            ->selectRaw($this->amountInNationalCurrency('ep.payment', 'e.currency_type_id', 'e.exchange_rate_sale').' as total')
            ->first();
    ...
    private function sumCanceledPurchases(array $valid_states)
    {
        $row = $this->currentTotalsBase()
            ...
            ->selectRaw($this->amountInNationalCurrency('p.total', 'p.currency_type_id', 'p.exchange_rate_sale').' as total')
            ->first();
    ...
    private function sumQuotations()
    {
        $row = $this->currentTotalsBase()
            ...
            ->selectRaw($this->amountInNationalCurrency('q.total', 'q.currency_type_id', 'q.exchange_rate_sale').' as total')
            ->first();
    ...
    private function sumFinanceIncomes(array $valid_states)
    {
        ...
        return (float) \Modules\Finance\Models\Income::where('user_id', $this->user_id)
            ...
            ->selectRaw($this->amountInNationalCurrency('total', 'currency_type_id', 'exchange_rate_sale').' as total')
            ->value('total');
    }
