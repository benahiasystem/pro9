 ...
 * (aperturas/cierres de caja, ventas y anulaciones) para registro local.
 */
class SyncController extends Controller
{
    ...
    public function enrollOptions()
    {
        ...
        // La conexión offline venezolana admite Factura y Nota de venta.
        $series = Series::where('dedicated', true)
            ->whereNull('series_device_group_id')
            ->whereIn('document_type_id', ['01', '80'])
        ...
        try {
            $result = DB::connection('tenant')->transaction(function () use ($request, $user, $plainToken) {
                ...
                foreach ($selected as $serie) {
                    if (!in_array($serie->document_type_id, ['01', '80'], true)) {
                        throw new \RuntimeException("La serie {$serie->number} no corresponde a Factura o Nota de venta");
                    }
                    if (!$serie->dedicated) {
    ...
    /** Datos fiscales básicos de la empresa para el comprobante local. */
    ...
    /** Establecimiento completo para el contexto comercial de la máquina. */
     ...
     *
     * Cada venta se valida y registra mediante la cadena común del facturador.
     * La operación local venezolana genera el PDF comercial, sin XML, CDR,
     * firma ni transmisión fiscal.
     */
