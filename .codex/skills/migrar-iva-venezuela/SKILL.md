...
5. Cambiar sólo presentación cuando el requisito sea terminológico. No renombrar identificadores internos ni alterar booleanos o payloads.
6. No editar bundles, mapas, minificados, manifests, vendor ni dependencias manualmente. Validar mediante el build de fuentes y conservar la salida sólo cuando el repositorio la versiona.
7. Delimitar cada unidad modificada con comentarios válidos que contengan exactamente `########## INICIO CAMBIO IGV A IVA`/`######### FIN CAMBIO IGV A IVA` para textos y `########## INICIO CAMBIO AFECTACIÓN IVA`/`######### FIN CAMBIO AFECTACIÓN IVA` para tasa o catálogo.
