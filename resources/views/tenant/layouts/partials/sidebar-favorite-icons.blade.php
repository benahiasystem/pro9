@php
    $sidebarFavoriteIconMarkup = [
        'adjustments' => '<path d="M4 10a2 2 0 1 0 4 0a2 2 0 0 0 -4 0" /> <path d="M6 4v4" /> <path d="M6 12v8" /> <path d="M10 16a2 2 0 1 0 4 0a2 2 0 0 0 -4 0" /> <path d="M12 4v10" /> <path d="M12 18v2" /> <path d="M16 7a2 2 0 1 0 4 0a2 2 0 0 0 -4 0" /> <path d="M18 4v1" /> <path d="M18 9v11" />',
        'alert-triangle' => '<path d="M12 9v4" /> <path d="M10.363 3.591l-8.106 13.534a1.914 1.914 0 0 0 1.636 2.871h16.214a1.914 1.914 0 0 0 1.636 -2.87l-8.106 -13.536a1.914 1.914 0 0 0 -3.274 0" /> <path d="M12 16h.01" />',
        'arrow-back-up' => '<path d="M9 14l-4 -4l4 -4" /> <path d="M5 10h11a4 4 0 1 1 0 8h-1" />',
        'arrows-exchange' => '<path d="M7 10h14l-4 -4" /> <path d="M17 14h-14l4 4" />',
        'arrows-left-right' => '<path d="M21 17l-18 0" /> <path d="M6 10l-3 -3l3 -3" /> <path d="M3 7l18 0" /> <path d="M18 20l3 -3l-3 -3" />',
        'basket' => '<path d="M10 14a2 2 0 1 0 4 0a2 2 0 0 0 -4 0" /> <path d="M5.001 8h13.999a2 2 0 0 1 1.977 2.304l-1.255 7.152a3 3 0 0 1 -2.966 2.544h-9.512a3 3 0 0 1 -2.965 -2.544l-1.255 -7.152a2 2 0 0 1 1.977 -2.304" /> <path d="M17 10l-2 -6" /> <path d="M7 10l2 -6" />',
        'bed' => '<path d="M5 9a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /> <path d="M22 17v-3h-20" /> <path d="M2 8v9" /> <path d="M12 14h10v-2a3 3 0 0 0 -3 -3h-7v5" />',
        'bell-ringing' => '<path d="M10 5a2 2 0 0 1 4 0a7 7 0 0 1 4 6v3a4 4 0 0 0 2 3h-16a4 4 0 0 0 2 -3v-3a7 7 0 0 1 4 -6" /> <path d="M9 17v1a3 3 0 0 0 6 0v-1" /> <path d="M21 6.727a11.05 11.05 0 0 0 -2.794 -3.727" /> <path d="M3 6.727a11.05 11.05 0 0 1 2.792 -3.727" />',
        'bolt' => '<path d="M13 3l0 7l6 0l-8 11l0 -7l-6 0l8 -11" />',
        'book' => '<path d="M3 19a9 9 0 0 1 9 0a9 9 0 0 1 9 0" /> <path d="M3 6a9 9 0 0 1 9 0a9 9 0 0 1 9 0" /> <path d="M3 6l0 13" /> <path d="M12 6l0 13" /> <path d="M21 6l0 13" />',
        'book-2' => '<path d="M19 4v16h-12a2 2 0 0 1 -2 -2v-12a2 2 0 0 1 2 -2h12" /> <path d="M19 16h-12a2 2 0 0 0 -2 2" /> <path d="M9 8h6" />',
        'box' => '<path d="M12 3l8 4.5l0 9l-8 4.5l-8 -4.5l0 -9l8 -4.5" /> <path d="M12 12l8 -4.5" /> <path d="M12 12l0 9" /> <path d="M12 12l-8 -4.5" />',
        'briefcase' => '<path d="M3 9a2 2 0 0 1 2 -2h14a2 2 0 0 1 2 2v9a2 2 0 0 1 -2 2h-14a2 2 0 0 1 -2 -2l0 -9" /> <path d="M8 7v-2a2 2 0 0 1 2 -2h4a2 2 0 0 1 2 2v2" /> <path d="M12 12l0 .01" /> <path d="M3 13a20 20 0 0 0 18 0" />',
        'building' => '<path d="M3 21l18 0" /> <path d="M9 8l1 0" /> <path d="M9 12l1 0" /> <path d="M9 16l1 0" /> <path d="M14 8l1 0" /> <path d="M14 12l1 0" /> <path d="M14 16l1 0" /> <path d="M5 21v-16a2 2 0 0 1 2 -2h10a2 2 0 0 1 2 2v16" />',
        'building-bank' => '<path d="M3 21l18 0" /> <path d="M3 10l18 0" /> <path d="M5 6l7 -3l7 3" /> <path d="M4 10l0 11" /> <path d="M20 10l0 11" /> <path d="M8 14l0 3" /> <path d="M12 14l0 3" /> <path d="M16 14l0 3" />',
        'building-factory' => '<path d="M4 21c1.147 -4.02 1.983 -8.027 2 -12h6c.017 3.973 .853 7.98 2 12" /> <path d="M12.5 13h4.5c.025 2.612 .894 5.296 2 8" /> <path d="M9 5a2.4 2.4 0 0 1 2 -1a2.4 2.4 0 0 1 2 1a2.4 2.4 0 0 0 2 1a2.4 2.4 0 0 0 2 -1a2.4 2.4 0 0 1 2 -1a2.4 2.4 0 0 1 2 1" /> <path d="M3 21l19 0" />',
        'building-warehouse' => '<path d="M3 21v-13l9 -4l9 4v13" /> <path d="M13 13h4v8h-10v-6h6" /> <path d="M13 21v-9a1 1 0 0 0 -1 -1h-2a1 1 0 0 0 -1 1v3" />',
        'calendar-dollar' => '<path d="M13 21h-7a2 2 0 0 1 -2 -2v-12a2 2 0 0 1 2 -2h12a2 2 0 0 1 2 2v3" /> <path d="M16 3v4" /> <path d="M8 3v4" /> <path d="M4 11h12.5" /> <path d="M21 15h-2.5a1.5 1.5 0 0 0 0 3h1a1.5 1.5 0 0 1 0 3h-2.5" /> <path d="M19 21v1m0 -8v1" />',
        'car' => '<path d="M5 17a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /> <path d="M15 17a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /> <path d="M5 17h-2v-6l2 -5h9l4 5h1a2 2 0 0 1 2 2v4h-2m-4 0h-6m-6 -6h15m-6 0v-5" />',
        'cards' => '<path d="M3.604 7.197l7.138 -3.109a.96 .96 0 0 1 1.27 .527l4.924 11.902a1 1 0 0 1 -.514 1.304l-7.137 3.109a.96 .96 0 0 1 -1.271 -.527l-4.924 -11.903a1 1 0 0 1 .514 -1.304l0 .001" /> <path d="M15 4h1a1 1 0 0 1 1 1v3.5" /> <path d="M20 6c.264 .112 .52 .217 .768 .315a1 1 0 0 1 .53 1.311l-2.298 5.374" />',
        'carrot' => '<path d="M3 21s9.834 -3.489 12.684 -6.34a4.487 4.487 0 0 0 0 -6.344a4.483 4.483 0 0 0 -6.342 0c-2.86 2.861 -6.347 12.689 -6.347 12.689l.005 -.005" /> <path d="M9 13l-1.5 -1.5" /> <path d="M16 14l-2 -2" /> <path d="M22 8s-1.14 -2 -3 -2c-1.406 0 -3 2 -3 2s1.14 2 3 2s3 -2 3 -2" /> <path d="M16 2s-2 1.14 -2 3s2 3 2 3s2 -1.577 2 -3c0 -1.86 -2 -3 -2 -3" />',
        'cash' => '<path d="M7 15h-3a1 1 0 0 1 -1 -1v-8a1 1 0 0 1 1 -1h12a1 1 0 0 1 1 1v3" /> <path d="M7 10a1 1 0 0 1 1 -1h12a1 1 0 0 1 1 1v8a1 1 0 0 1 -1 1h-12a1 1 0 0 1 -1 -1l0 -8" /> <path d="M12 14a2 2 0 1 0 4 0a2 2 0 0 0 -4 0" />',
        'cash-register' => '<path d="M21 15h-2.5c-.398 0 -.779 .158 -1.061 .439c-.281 .281 -.439 .663 -.439 1.061c0 .398 .158 .779 .439 1.061c.281 .281 .663 .439 1.061 .439h1c.398 0 .779 .158 1.061 .439c.281 .281 .439 .663 .439 1.061c0 .398 -.158 .779 -.439 1.061c-.281 .281 -.663 .439 -1.061 .439h-2.5" /> <path d="M19 21v1m0 -8v1" /> <path d="M13 21h-7c-.53 0 -1.039 -.211 -1.414 -.586c-.375 -.375 -.586 -.884 -.586 -1.414v-10c0 -.53 .211 -1.039 .586 -1.414c.375 -.375 .884 -.586 1.414 -.586h2m12 3.12v-1.12c0 -.53 -.211 -1.039 -.586 -1.414c-.375 -.375 -.884 -.586 -1.414 -.586h-2" /> <path d="M16 10v-6c0 -.53 -.211 -1.039 -.586 -1.414c-.375 -.375 -.884 -.586 -1.414 -.586h-4c-.53 0 -1.039 .211 -1.414 .586c-.375 .375 -.586 .884 -.586 1.414v6m8 0h-8m8 0h1m-9 0h-1" /> <path d="M8 14v.01" /> <path d="M8 17v.01" /> <path d="M12 13.99v.01" /> <path d="M12 17v.01" />',
        'category' => '<path d="M4 4h6v6h-6l0 -6" /> <path d="M14 4h6v6h-6l0 -6" /> <path d="M4 14h6v6h-6l0 -6" /> <path d="M14 17a3 3 0 1 0 6 0a3 3 0 1 0 -6 0" />',
        'category-2' => '<path d="M14 4h6v6h-6l0 -6" /> <path d="M4 14h6v6h-6l0 -6" /> <path d="M14 17a3 3 0 1 0 6 0a3 3 0 1 0 -6 0" /> <path d="M4 7a3 3 0 1 0 6 0a3 3 0 1 0 -6 0" />',
        'certificate' => '<path d="M12 15a3 3 0 1 0 6 0a3 3 0 1 0 -6 0" /> <path d="M13 17.5v4.5l2 -1.5l2 1.5v-4.5" /> <path d="M10 19h-5a2 2 0 0 1 -2 -2v-10c0 -1.1 .9 -2 2 -2h14a2 2 0 0 1 2 2v10a2 2 0 0 1 -1 1.73" /> <path d="M6 9l12 0" /> <path d="M6 12l3 0" /> <path d="M6 15l2 0" />',
        'chart-bar' => '<path d="M3 13a1 1 0 0 1 1 -1h4a1 1 0 0 1 1 1v6a1 1 0 0 1 -1 1h-4a1 1 0 0 1 -1 -1l0 -6" /> <path d="M15 9a1 1 0 0 1 1 -1h4a1 1 0 0 1 1 1v10a1 1 0 0 1 -1 1h-4a1 1 0 0 1 -1 -1l0 -10" /> <path d="M9 5a1 1 0 0 1 1 -1h4a1 1 0 0 1 1 1v14a1 1 0 0 1 -1 1h-4a1 1 0 0 1 -1 -1l0 -14" /> <path d="M4 20h14" />',
        'chart-histogram' => '<path d="M3 3v18h18" /> <path d="M20 18v3" /> <path d="M16 16v5" /> <path d="M12 13v8" /> <path d="M8 16v5" /> <path d="M3 11c6 0 5 -5 9 -5s3 5 9 5" />',
        'chart-pie' => '<path d="M10 3.2a9 9 0 1 0 10.8 10.8a1 1 0 0 0 -1 -1h-6.8a2 2 0 0 1 -2 -2v-7a.9 .9 0 0 0 -1 -.8" /> <path d="M15 3.5a9 9 0 0 1 5.5 5.5h-4.5a1 1 0 0 1 -1 -1v-4.5" />',
        'checklist' => '<path d="M9.615 20h-2.615a2 2 0 0 1 -2 -2v-12a2 2 0 0 1 2 -2h8a2 2 0 0 1 2 2v8" /> <path d="M14 19l2 2l4 -4" /> <path d="M9 8h4" /> <path d="M9 12h2" />',
        'click' => '<path d="M3 12l3 0" /> <path d="M12 3l0 3" /> <path d="M7.8 7.8l-2.2 -2.2" /> <path d="M16.2 7.8l2.2 -2.2" /> <path d="M7.8 16.2l-2.2 2.2" /> <path d="M12 12l9 3l-4 2l-2 4l-3 -9" />',
        'clipboard-list' => '<path d="M9 5h-2a2 2 0 0 0 -2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2 -2v-12a2 2 0 0 0 -2 -2h-2" /> <path d="M9 5a2 2 0 0 1 2 -2h2a2 2 0 0 1 2 2a2 2 0 0 1 -2 2h-2a2 2 0 0 1 -2 -2" /> <path d="M9 12l.01 0" /> <path d="M13 12l2 0" /> <path d="M9 16l.01 0" /> <path d="M13 16l2 0" />',
        'clipboard-text' => '<path d="M9 5h-2a2 2 0 0 0 -2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2 -2v-12a2 2 0 0 0 -2 -2h-2" /> <path d="M9 5a2 2 0 0 1 2 -2h2a2 2 0 0 1 2 2a2 2 0 0 1 -2 2h-2a2 2 0 0 1 -2 -2" /> <path d="M9 12h6" /> <path d="M9 16h6" />',
        'cloud-download' => '<path d="M19 18a3.5 3.5 0 0 0 0 -7h-1a5 4.5 0 0 0 -11 -2a4.6 4.4 0 0 0 -2.1 8.4" /> <path d="M12 13l0 9" /> <path d="M9 19l3 3l3 -3" />',
        'cloud-upload' => '<path d="M7 18a4.6 4.4 0 0 1 0 -9a5 4.5 0 0 1 11 2h1a3.5 3.5 0 0 1 0 7h-1" /> <path d="M9 15l3 -3l3 3" /> <path d="M12 12l0 9" />',
        'coin' => '<path d="M3 12a9 9 0 1 0 18 0a9 9 0 1 0 -18 0" /> <path d="M14.8 9a2 2 0 0 0 -1.8 -1h-2a2 2 0 1 0 0 4h2a2 2 0 1 1 0 4h-2a2 2 0 0 1 -1.8 -1" /> <path d="M12 7v10" />',
        'coins' => '<path d="M9 14c0 1.657 2.686 3 6 3s6 -1.343 6 -3s-2.686 -3 -6 -3s-6 1.343 -6 3" /> <path d="M9 14v4c0 1.656 2.686 3 6 3s6 -1.344 6 -3v-4" /> <path d="M3 6c0 1.072 1.144 2.062 3 2.598s4.144 .536 6 0c1.856 -.536 3 -1.526 3 -2.598c0 -1.072 -1.144 -2.062 -3 -2.598s-4.144 -.536 -6 0c-1.856 .536 -3 1.526 -3 2.598" /> <path d="M3 6v10c0 .888 .772 1.45 2 2" /> <path d="M3 11c0 .888 .772 1.45 2 2" />',
        'credit-card' => '<path d="M3 8a3 3 0 0 1 3 -3h12a3 3 0 0 1 3 3v8a3 3 0 0 1 -3 3h-12a3 3 0 0 1 -3 -3l0 -8" /> <path d="M3 10l18 0" /> <path d="M7 15l.01 0" /> <path d="M11 15l2 0" />',
        'door-enter' => '<path d="M13 12v.01" /> <path d="M3 21h18" /> <path d="M5 21v-16a2 2 0 0 1 2 -2h6m4 10.5v7.5" /> <path d="M21 7h-7m3 -3l-3 3l3 3" />',
        'engine' => '<path d="M3 10v6" /> <path d="M12 5v3" /> <path d="M10 5h4" /> <path d="M5 13h-2" /> <path d="M6 10h2l2 -2h3.382a1 1 0 0 1 .894 .553l1.448 2.894a1 1 0 0 0 .894 .553h1.382v-2h2a1 1 0 0 1 1 1v6a1 1 0 0 1 -1 1h-2v-2h-3v2a1 1 0 0 1 -1 1h-3.465a1 1 0 0 1 -.832 -.445l-1.703 -2.555h-2v-6" />',
        'external-link' => '<path d="M12 6h-6a2 2 0 0 0 -2 2v10a2 2 0 0 0 2 2h10a2 2 0 0 0 2 -2v-6" /> <path d="M11 13l9 -9" /> <path d="M15 4h5v5" />',
        'eye' => '<path d="M10 12a2 2 0 1 0 4 0a2 2 0 0 0 -4 0" /> <path d="M21 12c-2.4 4 -5.4 6 -9 6c-3.6 0 -6.6 -2 -9 -6c2.4 -4 5.4 -6 9 -6c3.6 0 6.6 2 9 6" />',
        'file-alert' => '<path d="M14 3v4a1 1 0 0 0 1 1h4" /> <path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2" /> <path d="M12 17l.01 0" /> <path d="M12 11l0 3" />',
        'file-certificate' => '<path d="M14 3v4a1 1 0 0 0 1 1h4" /> <path d="M5 8v-3a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2h-5" /> <path d="M3 14a3 3 0 1 0 6 0a3 3 0 1 0 -6 0" /> <path d="M4.5 17l-1.5 5l3 -1.5l3 1.5l-1.5 -5" />',
        'file-check' => '<path d="M14 3v4a1 1 0 0 0 1 1h4" /> <path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2" /> <path d="M9 15l2 2l4 -4" />',
        'file-description' => '<path d="M14 3v4a1 1 0 0 0 1 1h4" /> <path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2" /> <path d="M9 17h6" /> <path d="M9 13h6" />',
        'file-dollar' => '<path d="M14 3v4a1 1 0 0 0 1 1h4" /> <path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2" /> <path d="M14 11h-2.5a1.5 1.5 0 0 0 0 3h1a1.5 1.5 0 0 1 0 3h-2.5" /> <path d="M12 17v1m0 -8v1" />',
        'file-export' => '<path d="M14 3v4a1 1 0 0 0 1 1h4" /> <path d="M11.5 21h-4.5a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v5m-5 6h7m-3 -3l3 3l-3 3" />',
        'file-invoice' => '<path d="M14 3v4a1 1 0 0 0 1 1h4" /> <path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2" /> <path d="M9 7l1 0" /> <path d="M9 13l6 0" /> <path d="M13 17l2 0" />',
        'file-pencil' => '<path d="M14 3v4a1 1 0 0 0 1 1h4" /> <path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2" /> <path d="M10 18l5 -5a1.414 1.414 0 0 0 -2 -2l-5 5v2h2" />',
        'file-plus' => '<path d="M14 3v4a1 1 0 0 0 1 1h4" /> <path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2" /> <path d="M12 11l0 6" /> <path d="M9 14l6 0" />',
        'file-search' => '<path d="M14 3v4a1 1 0 0 0 1 1h4" /> <path d="M12 21h-5a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v4.5" /> <path d="M14 17.5a2.5 2.5 0 1 0 5 0a2.5 2.5 0 1 0 -5 0" /> <path d="M18.5 19.5l2.5 2.5" />',
        'file-spreadsheet' => '<path d="M14 3v4a1 1 0 0 0 1 1h4" /> <path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2" /> <path d="M8 11h8v7h-8l0 -7" /> <path d="M8 15h8" /> <path d="M11 11v7" />',
        'file-x' => '<path d="M14 3v4a1 1 0 0 0 1 1h4" /> <path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2" /> <path d="M10 12l4 4m0 -4l-4 4" />',
        'files' => '<path d="M15 3v4a1 1 0 0 0 1 1h4" /> <path d="M18 17h-7a2 2 0 0 1 -2 -2v-10a2 2 0 0 1 2 -2h4l5 5v7a2 2 0 0 1 -2 2" /> <path d="M16 17v2a2 2 0 0 1 -2 2h-7a2 2 0 0 1 -2 -2v-10a2 2 0 0 1 2 -2h2" />',
        'flag' => '<path d="M5 5a5 5 0 0 1 7 0a5 5 0 0 0 7 0v9a5 5 0 0 1 -7 0a5 5 0 0 0 -7 0v-9" /> <path d="M5 21v-7" />',
        'folder' => '<path d="M5 4h4l3 3h7a2 2 0 0 1 2 2v8a2 2 0 0 1 -2 2h-14a2 2 0 0 1 -2 -2v-11a2 2 0 0 1 2 -2" />',
        'folders' => '<path d="M9 3h3l2 2h5a2 2 0 0 1 2 2v7a2 2 0 0 1 -2 2h-10a2 2 0 0 1 -2 -2v-9a2 2 0 0 1 2 -2" /> <path d="M17 16v2a2 2 0 0 1 -2 2h-10a2 2 0 0 1 -2 -2v-9a2 2 0 0 1 2 -2h2" />',
        'git-branch' => '<path d="M5 18a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /> <path d="M5 6a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /> <path d="M15 6a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /> <path d="M7 8l0 8" /> <path d="M9 18h6a2 2 0 0 0 2 -2v-5" /> <path d="M14 14l3 -3l3 3" />',
        'hammer' => '<path d="M11.414 10l-7.383 7.418a2.091 2.091 0 0 0 0 2.967a2.11 2.11 0 0 0 2.976 0l7.407 -7.385" /> <path d="M18.121 15.293l2.586 -2.586a1 1 0 0 0 0 -1.414l-7.586 -7.586a1 1 0 0 0 -1.414 0l-2.586 2.586a1 1 0 0 0 0 1.414l7.586 7.586a1 1 0 0 0 1.414 0" />',
        'hourglass' => '<path d="M6.5 7h11" /> <path d="M6.5 17h11" /> <path d="M6 20v-2a6 6 0 1 1 12 0v2a1 1 0 0 1 -1 1h-10a1 1 0 0 1 -1 -1" /> <path d="M6 4v2a6 6 0 1 0 12 0v-2a1 1 0 0 0 -1 -1h-10a1 1 0 0 0 -1 1" />',
        'id-badge-2' => '<path d="M7 12h3v4h-3l0 -4" /> <path d="M10 6h-6a1 1 0 0 0 -1 1v12a1 1 0 0 0 1 1h16a1 1 0 0 0 1 -1v-12a1 1 0 0 0 -1 -1h-6" /> <path d="M10 4a1 1 0 0 1 1 -1h2a1 1 0 0 1 1 1v3a1 1 0 0 1 -1 1h-2a1 1 0 0 1 -1 -1l0 -3" /> <path d="M14 16h2" /> <path d="M14 12h4" />',
        'list-check' => '<path d="M3.5 5.5l1.5 1.5l2.5 -2.5" /> <path d="M3.5 11.5l1.5 1.5l2.5 -2.5" /> <path d="M3.5 17.5l1.5 1.5l2.5 -2.5" /> <path d="M11 6l9 0" /> <path d="M11 12l9 0" /> <path d="M11 18l9 0" />',
        'list-details' => '<path d="M13 5h8" /> <path d="M13 9h5" /> <path d="M13 15h8" /> <path d="M13 19h5" /> <path d="M3 5a1 1 0 0 1 1 -1h4a1 1 0 0 1 1 1v4a1 1 0 0 1 -1 1h-4a1 1 0 0 1 -1 -1l0 -4" /> <path d="M3 15a1 1 0 0 1 1 -1h4a1 1 0 0 1 1 1v4a1 1 0 0 1 -1 1h-4a1 1 0 0 1 -1 -1l0 -4" />',
        'list-numbers' => '<path d="M11 6h9" /> <path d="M11 12h9" /> <path d="M12 18h8" /> <path d="M4 16a2 2 0 1 1 4 0c0 .591 -.5 1 -1 1.5l-3 2.5h4" /> <path d="M6 10v-6l-2 2" />',
        'map-pin' => '<path d="M9 11a3 3 0 1 0 6 0a3 3 0 0 0 -6 0" /> <path d="M17.657 16.657l-4.243 4.243a2 2 0 0 1 -2.827 0l-4.244 -4.243a8 8 0 1 1 11.314 0" />',
        'message-report' => '<path d="M18 4a3 3 0 0 1 3 3v8a3 3 0 0 1 -3 3h-5l-5 3v-3h-2a3 3 0 0 1 -3 -3v-8a3 3 0 0 1 3 -3h12" /> <path d="M12 8v3" /> <path d="M12 14v.01" />',
        'mood-kid' => '<path d="M3 12a9 9 0 1 0 18 0a9 9 0 1 0 -18 0" /> <path d="M9 10l.01 0" /> <path d="M15 10l.01 0" /> <path d="M9.5 15a3.5 3.5 0 0 0 5 0" /> <path d="M12 3a2 2 0 0 0 0 4" />',
        'package' => '<path d="M12 3l8 4.5l0 9l-8 4.5l-8 -4.5l0 -9l8 -4.5" /> <path d="M12 12l8 -4.5" /> <path d="M12 12l0 9" /> <path d="M12 12l-8 -4.5" /> <path d="M16 5.25l-8 4.5" />',
        'package-import' => '<path d="M12 21l-8 -4.5v-9l8 -4.5l8 4.5v4.5" /> <path d="M12 12l8 -4.5" /> <path d="M12 12v9" /> <path d="M12 12l-8 -4.5" /> <path d="M22 18h-7" /> <path d="M18 15l-3 3l3 3" />',
        'packages' => '<path d="M7 16.5l-5 -3l5 -3l5 3v5.5l-5 3l0 -5.5" /> <path d="M2 13.5v5.5l5 3" /> <path d="M7 16.545l5 -3.03" /> <path d="M17 16.5l-5 -3l5 -3l5 3v5.5l-5 3l0 -5.5" /> <path d="M12 19l5 3" /> <path d="M17 16.5l5 -3" /> <path d="M12 13.5v-5.5l-5 -3l5 -3l5 3v5.5" /> <path d="M7 5.03v5.455" /> <path d="M12 8l5 -3" />',
        'percentage' => '<path d="M16 17a1 1 0 1 0 2 0a1 1 0 1 0 -2 0" /> <path d="M6 7a1 1 0 1 0 2 0a1 1 0 1 0 -2 0" /> <path d="M6 18l12 -12" />',
        'photo' => '<path d="M15 8h.01" /> <path d="M3 6a3 3 0 0 1 3 -3h12a3 3 0 0 1 3 3v12a3 3 0 0 1 -3 3h-12a3 3 0 0 1 -3 -3v-12" /> <path d="M3 16l5 -5c.928 -.893 2.072 -.893 3 0l5 5" /> <path d="M14 14l1 -1c.928 -.893 2.072 -.893 3 0l3 3" />',
        'pill' => '<path d="M4.5 12.5l8 -8a4.94 4.94 0 0 1 7 7l-8 8a4.94 4.94 0 0 1 -7 -7" /> <path d="M8.5 8.5l7 7" />',
        'point' => '<path d="M8 12a4 4 0 1 0 8 0a4 4 0 1 0 -8 0" />',
        'receipt' => '<path d="M5 21v-16a2 2 0 0 1 2 -2h10a2 2 0 0 1 2 2v16l-3 -2l-2 2l-2 -2l-2 2l-2 -2l-3 2m4 -14h6m-6 4h6m-2 4h2" />',
        'receipt-tax' => '<path d="M9 14l6 -6" /> <path d="M9 8.5a.5 .5 0 1 0 1 0a.5 .5 0 1 0 -1 0" fill="currentColor" /> <path d="M14 13.5a.5 .5 0 1 0 1 0a.5 .5 0 1 0 -1 0" fill="currentColor" /> <path d="M5 21v-16a2 2 0 0 1 2 -2h10a2 2 0 0 1 2 2v16l-3 -2l-2 2l-2 -2l-2 2l-2 -2l-3 2" />',
        'repeat' => '<path d="M4 12v-3a3 3 0 0 1 3 -3h13m-3 -3l3 3l-3 3" /> <path d="M20 12v3a3 3 0 0 1 -3 3h-13m3 3l-3 -3l3 -3" />',
        'report' => '<path d="M8 5h-2a2 2 0 0 0 -2 2v12a2 2 0 0 0 2 2h5.697" /> <path d="M18 14v4h4" /> <path d="M18 11v-4a2 2 0 0 0 -2 -2h-2" /> <path d="M8 5a2 2 0 0 1 2 -2h2a2 2 0 0 1 2 2a2 2 0 0 1 -2 2h-2a2 2 0 0 1 -2 -2" /> <path d="M14 18a4 4 0 1 0 8 0a4 4 0 1 0 -8 0" /> <path d="M8 11h4" /> <path d="M8 15h3" />',
        'report-analytics' => '<path d="M9 5h-2a2 2 0 0 0 -2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2 -2v-12a2 2 0 0 0 -2 -2h-2" /> <path d="M9 5a2 2 0 0 1 2 -2h2a2 2 0 0 1 2 2a2 2 0 0 1 -2 2h-2a2 2 0 0 1 -2 -2" /> <path d="M9 17v-5" /> <path d="M12 17v-1" /> <path d="M15 17v-3" />',
        'report-money' => '<path d="M9 5h-2a2 2 0 0 0 -2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2 -2v-12a2 2 0 0 0 -2 -2h-2" /> <path d="M9 5a2 2 0 0 1 2 -2h2a2 2 0 0 1 2 2a2 2 0 0 1 -2 2h-2a2 2 0 0 1 -2 -2" /> <path d="M14 11h-2.5a1.5 1.5 0 0 0 0 3h1a1.5 1.5 0 0 1 0 3h-2.5" /> <path d="M12 17v1m0 -8v1" />',
        'route' => '<path d="M3 19a2 2 0 1 0 4 0a2 2 0 0 0 -4 0" /> <path d="M19 7a2 2 0 1 0 0 -4a2 2 0 0 0 0 4" /> <path d="M11 19h5.5a3.5 3.5 0 0 0 0 -7h-8a3.5 3.5 0 0 1 0 -7h4.5" />',
        'scale' => '<path d="M7 20l10 0" /> <path d="M6 6l6 -1l6 1" /> <path d="M12 3l0 17" /> <path d="M9 12l-3 -6l-3 6a3 3 0 0 0 6 0" /> <path d="M21 12l-3 -6l-3 6a3 3 0 0 0 6 0" />',
        'school' => '<path d="M22 9l-10 -4l-10 4l10 4l10 -4v6" /> <path d="M6 10.6v5.4a6 3 0 0 0 12 0v-5.4" />',
        'settings' => '<path d="M10.325 4.317c.426 -1.756 2.924 -1.756 3.35 0a1.724 1.724 0 0 0 2.573 1.066c1.543 -.94 3.31 .826 2.37 2.37a1.724 1.724 0 0 0 1.065 2.572c1.756 .426 1.756 2.924 0 3.35a1.724 1.724 0 0 0 -1.066 2.573c.94 1.543 -.826 3.31 -2.37 2.37a1.724 1.724 0 0 0 -2.572 1.065c-.426 1.756 -2.924 1.756 -3.35 0a1.724 1.724 0 0 0 -2.573 -1.066c-1.543 .94 -3.31 -.826 -2.37 -2.37a1.724 1.724 0 0 0 -1.065 -2.572c-1.756 -.426 -1.756 -2.924 0 -3.35a1.724 1.724 0 0 0 1.066 -2.573c-.94 -1.543 .826 -3.31 2.37 -2.37c1 .608 2.296 .07 2.572 -1.065" /> <path d="M9 12a3 3 0 1 0 6 0a3 3 0 0 0 -6 0" />',
        'settings-2' => '<path d="M19.875 6.27a2.225 2.225 0 0 1 1.125 1.948v7.284c0 .809 -.443 1.555 -1.158 1.948l-6.75 4.27a2.269 2.269 0 0 1 -2.184 0l-6.75 -4.27a2.225 2.225 0 0 1 -1.158 -1.948v-7.285c0 -.809 .443 -1.554 1.158 -1.947l6.75 -3.98a2.33 2.33 0 0 1 2.25 0l6.75 3.98h-.033" /> <path d="M9 12a3 3 0 1 0 6 0a3 3 0 1 0 -6 0" />',
        'settings-automation' => '<path d="M10.325 4.317c.426 -1.756 2.924 -1.756 3.35 0a1.724 1.724 0 0 0 2.573 1.066c1.543 -.94 3.31 .826 2.37 2.37a1.724 1.724 0 0 0 1.065 2.572c1.756 .426 1.756 2.924 0 3.35a1.724 1.724 0 0 0 -1.066 2.573c.94 1.543 -.826 3.31 -2.37 2.37a1.724 1.724 0 0 0 -2.572 1.065c-.426 1.756 -2.924 1.756 -3.35 0a1.724 1.724 0 0 0 -2.573 -1.066c-1.543 .94 -3.31 -.826 -2.37 -2.37a1.724 1.724 0 0 0 -1.065 -2.572c-1.756 -.426 -1.756 -2.924 0 -3.35a1.724 1.724 0 0 0 1.066 -2.573c-.94 -1.543 .826 -3.31 2.37 -2.37c1 .608 2.296 .07 2.572 -1.065" /> <path d="M10 9v6l5 -3l-5 -3" />',
        'shopping-bag' => '<path d="M6.331 8h11.339a2 2 0 0 1 1.977 2.304l-1.255 8.152a3 3 0 0 1 -2.966 2.544h-6.852a3 3 0 0 1 -2.965 -2.544l-1.255 -8.152a2 2 0 0 1 1.977 -2.304" /> <path d="M9 11v-5a3 3 0 0 1 6 0v5" />',
        'shopping-cart' => '<path d="M4 19a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /> <path d="M15 19a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /> <path d="M17 17h-11v-14h-2" /> <path d="M6 5l14 1l-1 7h-13" />',
        'shopping-cart-plus' => '<path d="M4 19a2 2 0 1 0 4 0a2 2 0 0 0 -4 0" /> <path d="M12.5 17h-6.5v-14h-2" /> <path d="M6 5l14 1l-.86 6.017m-2.64 .983h-10.5" /> <path d="M16 19h6" /> <path d="M19 16v6" />',
        'stairs' => '<path d="M22 5h-5v5h-5v5h-5v5h-5" />',
        'steering-wheel' => '<path d="M3 12a9 9 0 1 0 18 0a9 9 0 1 0 -18 0" /> <path d="M10 12a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /> <path d="M12 14l0 7" /> <path d="M10 12l-6.75 -2" /> <path d="M14 12l6.75 -2" />',
        'tag' => '<path d="M6.5 7.5a1 1 0 1 0 2 0a1 1 0 1 0 -2 0" /> <path d="M3 6v5.172a2 2 0 0 0 .586 1.414l7.71 7.71a2.41 2.41 0 0 0 3.408 0l5.592 -5.592a2.41 2.41 0 0 0 0 -3.408l-7.71 -7.71a2 2 0 0 0 -1.414 -.586h-5.172a3 3 0 0 0 -3 3" />',
        'tags' => '<path d="M3 8v4.172a2 2 0 0 0 .586 1.414l5.71 5.71a2.41 2.41 0 0 0 3.408 0l3.592 -3.592a2.41 2.41 0 0 0 0 -3.408l-5.71 -5.71a2 2 0 0 0 -1.414 -.586h-4.172a2 2 0 0 0 -2 2" /> <path d="M18 19l1.592 -1.592a4.82 4.82 0 0 0 0 -6.816l-4.592 -4.592" /> <path d="M7 10h-.01" />',
        'target' => '<path d="M11 12a1 1 0 1 0 2 0a1 1 0 1 0 -2 0" /> <path d="M7 12a5 5 0 1 0 10 0a5 5 0 1 0 -10 0" /> <path d="M3 12a9 9 0 1 0 18 0a9 9 0 1 0 -18 0" />',
        'tool' => '<path d="M7 10h3v-3l-3.5 -3.5a6 6 0 0 1 8 8l6 6a2 2 0 0 1 -3 3l-6 -6a6 6 0 0 1 -8 -8l3.5 3.5" />',
        'tools-kitchen-2' => '<path d="M19 3v12h-5c-.023 -3.681 .184 -7.406 5 -12m0 12v6h-1v-3m-10 -14v17m-3 -17v3a3 3 0 1 0 6 0v-3" />',
        'transfer' => '<path d="M20 10h-16l5.5 -6" /> <path d="M4 14h16l-5.5 6" />',
        'trending-up' => '<path d="M3 17l6 -6l4 4l8 -8" /> <path d="M14 7l7 0l0 7" />',
        'truck' => '<path d="M5 17a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /> <path d="M15 17a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /> <path d="M5 17h-2v-11a1 1 0 0 1 1 -1h9v12m-4 0h6m4 0h2v-6h-8m0 -5h5l3 5" />',
        'truck-delivery' => '<path d="M5 17a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /> <path d="M15 17a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /> <path d="M5 17h-2v-4m-1 -8h11v12m-4 0h6m4 0h2v-6h-8m0 -5h5l3 5" /> <path d="M3 9l4 0" />',
        'user-cog' => '<path d="M8 7a4 4 0 1 0 8 0a4 4 0 0 0 -8 0" /> <path d="M6 21v-2a4 4 0 0 1 4 -4h2.5" /> <path d="M17.001 19a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /> <path d="M19.001 15.5v1.5" /> <path d="M19.001 21v1.5" /> <path d="M22.032 17.25l-1.299 .75" /> <path d="M17.27 20l-1.3 .75" /> <path d="M15.97 17.25l1.3 .75" /> <path d="M20.733 20l1.3 .75" />',
        'user-star' => '<path d="M8 7a4 4 0 1 0 8 0a4 4 0 0 0 -8 0" /> <path d="M6 21v-2a4 4 0 0 1 4 -4h.5" /> <path d="M17.8 20.817l-2.172 1.138a.392 .392 0 0 1 -.568 -.41l.415 -2.411l-1.757 -1.707a.389 .389 0 0 1 .217 -.665l2.428 -.352l1.086 -2.193a.392 .392 0 0 1 .702 0l1.086 2.193l2.428 .352a.39 .39 0 0 1 .217 .665l-1.757 1.707l.414 2.41a.39 .39 0 0 1 -.567 .411l-2.172 -1.138" />',
        'users' => '<path d="M5 7a4 4 0 1 0 8 0a4 4 0 1 0 -8 0" /> <path d="M3 21v-2a4 4 0 0 1 4 -4h4a4 4 0 0 1 4 4v2" /> <path d="M16 3.13a4 4 0 0 1 0 7.75" /> <path d="M21 21v-2a4 4 0 0 0 -3 -3.85" />',
        'users-group' => '<path d="M10 13a2 2 0 1 0 4 0a2 2 0 0 0 -4 0" /> <path d="M8 21v-1a2 2 0 0 1 2 -2h4a2 2 0 0 1 2 2v1" /> <path d="M15 5a2 2 0 1 0 4 0a2 2 0 0 0 -4 0" /> <path d="M17 10h2a2 2 0 0 1 2 2v1" /> <path d="M5 5a2 2 0 1 0 4 0a2 2 0 0 0 -4 0" /> <path d="M3 13v-1a2 2 0 0 1 2 -2h2" />',
        'wallet' => '<path d="M17 8v-3a1 1 0 0 0 -1 -1h-10a2 2 0 0 0 0 4h12a1 1 0 0 1 1 1v3m0 4v3a1 1 0 0 1 -1 1h-12a2 2 0 0 1 -2 -2v-12" /> <path d="M20 12v4h-4a2 2 0 0 1 0 -4h4" />',
    ];

    $sidebarFavoriteIconEntries = [
        ['kind' => 'route', 'target' => 'tenant.sale_opportunities.index', 'icon' => 'target'], // Oportunidad de venta
        ['kind' => 'route', 'target' => 'tenant.quotations.index', 'icon' => 'file-dollar'], // Cotizaciones
        ['kind' => 'route', 'target' => 'tenant.contracts.index', 'icon' => 'file-certificate'], // Contratos
        ['kind' => 'route', 'target' => 'tenant.order_notes.index', 'icon' => 'clipboard-list'], // Pedidos
        ['kind' => 'route', 'target' => 'tenant.technical_services.index', 'icon' => 'tool'], // Servicio de soporte tecnico
        ['kind' => 'route', 'target' => 'tenant.documents.create', 'icon' => 'file-plus'], // Nuevo comprobante
        ['kind' => 'route', 'target' => 'tenant.documents.index', 'icon' => 'file-invoice'], // Boleta/factura
        ['kind' => 'route', 'target' => 'tenant.sale_notes.index', 'icon' => 'receipt'], // Notas de venta
        ['kind' => 'route', 'target' => 'tenant.pos.index', 'icon' => 'cash-register'], // Punto de venta
        ['kind' => 'route', 'target' => 'tenant.pos.garage', 'icon' => 'bolt'], // Venta rapida
        ['kind' => 'route', 'target' => 'tenant.purchases.create', 'icon' => 'shopping-cart-plus'], // Nueva compra
        ['kind' => 'route', 'target' => 'tenant.purchases.index', 'icon' => 'shopping-cart'], // Listado de compras
        ['kind' => 'route', 'target' => 'tenant.purchase-orders.index', 'icon' => 'shopping-bag'], // Ordenes de compra
        ['kind' => 'route', 'target' => 'tenant.expenses.index', 'icon' => 'coins'], // Gastos diversos
        ['kind' => 'route', 'target' => 'tenant.persons.index', 'params' => ['type' => 'suppliers'], 'icon' => 'truck-delivery'], // Proveedores
        ['kind' => 'route', 'target' => 'tenant.purchase-quotations.index', 'icon' => 'file-search'], // Solicitar cotizacion
        ['kind' => 'route', 'target' => 'tenant.fixed_asset_items.index', 'icon' => 'briefcase'], // Activos fijos
        ['kind' => 'route', 'target' => 'tenant.fixed_asset_purchases.index', 'icon' => 'package-import'], // Comprar activo fijo
        ['kind' => 'route', 'target' => 'tenant.persons.index', 'params' => ['type' => 'customers'], 'icon' => 'users'], // Clientes
        ['kind' => 'route', 'target' => 'tenant.person_types.index', 'icon' => 'user-cog'], // Tipos de clientes
        ['kind' => 'route', 'target' => 'tenant.agents.index', 'icon' => 'user-star'], // Agentes
        ['kind' => 'route', 'target' => 'tenant.items.index', 'icon' => 'package'], // Productos
        ['kind' => 'route', 'target' => 'tenant.item_sets.index', 'icon' => 'packages'], // Conjuntos y packs
        ['kind' => 'route', 'target' => 'tenant.services', 'icon' => 'hammer'], // Servicios
        ['kind' => 'route', 'target' => 'tenant.categories.index', 'icon' => 'category'], // Categorias
        ['kind' => 'route', 'target' => 'tenant.brands.index', 'icon' => 'tag'], // Marcas
        ['kind' => 'route', 'target' => 'tenant.item-lots.index', 'icon' => 'list-numbers'], // Series
        ['kind' => 'route', 'target' => 'tenant.zone.index', 'icon' => 'map-pin'], // Zonas
        ['kind' => 'route', 'target' => 'inventory.index', 'icon' => 'arrows-exchange'], // Movimientos de inventario
        ['kind' => 'route', 'target' => 'transfers.index', 'icon' => 'transfer'], // Traslados
        ['kind' => 'route', 'target' => 'devolutions.index', 'icon' => 'arrow-back-up'], // Devolucion a proveedor
        ['kind' => 'route', 'target' => 'reports.kardex.index', 'icon' => 'report-analytics'], // Reporte kardex
        ['kind' => 'route', 'target' => 'reports.inventory.index', 'icon' => 'report'], // Reporte inventario
        ['kind' => 'route', 'target' => 'warehouses.index', 'icon' => 'building-warehouse'], // Almacenes
        ['kind' => 'route', 'target' => 'reports.valued_kardex.index', 'icon' => 'report-money'], // Kardex valorizado
        ['kind' => 'route', 'target' => 'extra_info_items.index', 'icon' => 'list-details'], // Datos extra de items
        ['kind' => 'route', 'target' => 'tenant.inventory-review.index', 'icon' => 'checklist'], // Revision de inventario
        ['kind' => 'route', 'target' => 'tenant.cash.index', 'icon' => 'cash'], // Caja general
        ['kind' => 'route', 'target' => 'tenant.finances.movements.index', 'icon' => 'arrows-left-right'], // Movimientos
        ['kind' => 'route', 'target' => 'tenant.finances.transactions.index', 'icon' => 'credit-card'], // Transacciones
        ['kind' => 'route', 'target' => 'tenant.finances.income.index', 'icon' => 'trending-up'], // Ingresos
        ['kind' => 'route', 'target' => 'tenant.finances.unpaid.index', 'icon' => 'calendar-dollar'], // Cuentas por cobrar
        ['kind' => 'route', 'target' => 'tenant.finances.to_pay.index', 'icon' => 'hourglass'], // Cuentas por pagar
        ['kind' => 'route', 'target' => 'tenant.finances.global_payments.index', 'icon' => 'wallet'], // Pagos
        ['kind' => 'route', 'target' => 'tenant.finances.balance.index', 'icon' => 'scale'], // Balance
        ['kind' => 'route', 'target' => 'tenant.finances.payment_method_types.index', 'icon' => 'coin'], // Ingresos y egresos - M. Pago
        ['kind' => 'route', 'target' => 'tenant.bank_loan.index', 'icon' => 'building-bank'], // Credito bancario
        ['kind' => 'route', 'target' => 'tenant.dispatches.index', 'icon' => 'truck'], // Orden de entrega
        ['kind' => 'route', 'target' => 'tenant.dispatchers.index', 'icon' => 'users-group'], // Transportistas
        ['kind' => 'route', 'target' => 'tenant.drivers.index', 'icon' => 'id-badge-2'], // Conductores
        ['kind' => 'route', 'target' => 'tenant.transports.index', 'icon' => 'car'], // Vehiculos
        ['kind' => 'route', 'target' => 'tenant.origin_addresses.index', 'icon' => 'map-pin'], // Direcciones de partida
        ['kind' => 'route', 'target' => 'tenant.dispatch-addresses.index', 'icon' => 'route'], // Direcciones de llegada
        ['kind' => 'route', 'target' => 'tenant.documents.not_sent', 'icon' => 'file-alert'], // Comprobantes no enviados
        ['kind' => 'route', 'target' => 'tenant.documents.regularize_shipping', 'icon' => 'file-pencil'], // CPE pendientes de rectificacion
        ['kind' => 'route', 'target' => 'tenant.summaries.index', 'icon' => 'files'], // Resumenes
        ['kind' => 'route', 'target' => 'tenant.voided.index', 'icon' => 'file-x'], // Anulaciones
        ['kind' => 'route', 'target' => 'tenant.retentions.index', 'icon' => 'percentage'], // Retenciones
        ['kind' => 'route', 'target' => 'tenant.perceptions.index', 'icon' => 'receipt-tax'], // Percepciones
        ['kind' => 'route', 'target' => 'tenant.purchase-settlements.index', 'icon' => 'file-check'], // Liquidaciones de compra
        ['kind' => 'route', 'target' => 'tenant.order_forms.index', 'icon' => 'clipboard-text'], // Ordenes de pedido
        ['kind' => 'route', 'target' => 'tenant.contingencies.index', 'icon' => 'alert-triangle'], // Documentos de contingencia
        ['kind' => 'route', 'target' => 'tenant.account_format.index', 'icon' => 'file-export'], // Exportar reporte
        ['kind' => 'route', 'target' => 'tenant.account.index', 'icon' => 'file-spreadsheet'], // Exportar formatos - Sis. contable
        ['kind' => 'route', 'target' => 'tenant.account_summary_report.index', 'icon' => 'chart-bar'], // Reporte resumido - Ventas
        ['kind' => 'route', 'target' => 'tenant.accounting_ledger.create', 'icon' => 'book'], // Libro mayor
        ['kind' => 'route', 'target' => 'tenant.sire.sale', 'icon' => 'cloud-upload'], // SIRE ventas
        ['kind' => 'route', 'target' => 'tenant.sire.purchase', 'icon' => 'cloud-download'], // SIRE compras
        ['kind' => 'route', 'target' => 'tenant_orders_index', 'icon' => 'basket'], // Pedidos ecommerce
        ['kind' => 'route', 'target' => 'tenant.items_ecommerce.index', 'icon' => 'eye'], // Productos visibles
        ['kind' => 'route', 'target' => 'tenant.ecommerce.item_sets.index', 'icon' => 'packages'], // Conjuntos y packs ecommerce
        ['kind' => 'route', 'target' => 'tenant.tags.index', 'icon' => 'tags'], // Etiquetas
        ['kind' => 'route', 'target' => 'tenant_ecommerce_configuration', 'icon' => 'settings'], // Configuracion ecommerce
        ['kind' => 'route', 'target' => 'tenant.restaurant.cash.filter-pos', 'icon' => 'cash'], // Caja chica POS
        ['kind' => 'route', 'target' => 'tenant.restaurant.cash.index', 'icon' => 'cash'], // Caja chica mesas
        ['kind' => 'route', 'target' => 'tenant.restaurant.list_items', 'icon' => 'tools-kitchen-2'], // Productos restaurante
        ['kind' => 'route', 'target' => 'tenant.restaurant.supplies.index', 'icon' => 'carrot'], // Insumos
        ['kind' => 'url', 'target' => 'restaurant/modifier-groups', 'icon' => 'adjustments'], // Modificadores
        ['kind' => 'route', 'target' => 'tenant.restaurant.menu', 'icon' => 'external-link'], // Ver pedidos en linea
        ['kind' => 'route', 'target' => 'tenant.restaurant.order.index', 'icon' => 'list-check'], // Listado de pedidos
        ['kind' => 'route', 'target' => 'tenant.restaurant.promotion.index', 'icon' => 'photo'], // Promociones (banners)
        ['kind' => 'route', 'target' => 'tenant.restaurant.configuration', 'icon' => 'settings'], // Configuracion restaurante
        ['kind' => 'route', 'target' => 'tenant.digemid.index', 'icon' => 'pill'], // Productos digemid
        ['kind' => 'route', 'target' => 'tenant.configuration.index', 'icon' => 'settings-2'], // Configuracion
        ['kind' => 'route', 'target' => 'tenant.payment.index', 'icon' => 'credit-card'], // Lista de pagos
        ['kind' => 'route', 'target' => 'tenant.payment.generate.index', 'icon' => 'credit-card'], // Generar pago
        ['kind' => 'url', 'target' => 'hotels/reception', 'icon' => 'door-enter'], // Recepcion
        ['kind' => 'url', 'target' => 'hotels/rates', 'icon' => 'coin'], // Tarifas
        ['kind' => 'url', 'target' => 'hotels/floors', 'icon' => 'stairs'], // Ubicaciones
        ['kind' => 'url', 'target' => 'hotels/categories', 'icon' => 'category-2'], // Categorias de habitacion
        ['kind' => 'url', 'target' => 'hotels/rooms', 'icon' => 'bed'], // Habitaciones
        ['kind' => 'route', 'target' => 'tenant.fullsuscription.payments.index', 'icon' => 'repeat'], // Suscripciones
        ['kind' => 'route', 'target' => 'tenant.fullsuscription.payment_receipt.index', 'icon' => 'receipt'], // Recibos de pago
        ['kind' => 'route', 'target' => 'tenant.fullsuscription.plans.index', 'icon' => 'cards'], // Planes
        ['kind' => 'route', 'target' => 'tenant.full_suscription.pending-payments.index', 'icon' => 'hourglass'], // Pagos pendientes
        ['kind' => 'route', 'target' => 'tenant.full_suscription.payment-reminders.index', 'icon' => 'bell-ringing'], // Recordatorios de pago
        ['kind' => 'route', 'target' => 'tenant.suscription.client.index', 'icon' => 'users'], // Padres
        ['kind' => 'route', 'target' => 'tenant.suscription.client_children.index', 'icon' => 'mood-kid'], // Hijos
        ['kind' => 'route', 'target' => 'tenant.suscription.service.index', 'icon' => 'school'], // Servicio
        ['kind' => 'route', 'target' => 'tenant.suscription.plans.index', 'icon' => 'cards'], // Planes de suscripcion
        ['kind' => 'route', 'target' => 'tenant.suscription.payments.index', 'icon' => 'certificate'], // Matriculas
        ['kind' => 'route', 'target' => 'tenant.suscription.payment_receipt.index', 'icon' => 'receipt'], // Recibos de pago suscripcion
        ['kind' => 'route', 'target' => 'tenant.suscription.grade_section.index', 'icon' => 'book-2'], // Grados y secciones
        ['kind' => 'route', 'target' => 'documentary.offices', 'icon' => 'building'], // Oficinas
        ['kind' => 'route', 'target' => 'documentary.status', 'icon' => 'flag'], // Listado de estados
        ['kind' => 'route', 'target' => 'documentary.requirements', 'icon' => 'checklist'], // Listado de requisitos
        ['kind' => 'route', 'target' => 'documentary.processes', 'icon' => 'git-branch'], // Tipos de tramites
        ['kind' => 'route', 'target' => 'documentary.documents', 'icon' => 'file-description'], // Tipos de documento
        ['kind' => 'route', 'target' => 'documentary.actions', 'icon' => 'click'], // Acciones
        ['kind' => 'route', 'target' => 'documentary.files', 'icon' => 'folders'], // Listado de tramites
        ['kind' => 'route', 'target' => 'documentary.files_simplify', 'icon' => 'folder'], // Listado de tramites simplificado
        ['kind' => 'route', 'target' => 'documentary.stadistic', 'icon' => 'chart-pie'], // Estadisticas de tramites
        ['kind' => 'url', 'target' => 'claims', 'icon' => 'message-report'], // Libro de reclamaciones
        ['kind' => 'url', 'target' => 'list-reports', 'icon' => 'chart-histogram'], // Reportes
        ['kind' => 'route', 'target' => 'tenant.production.index', 'icon' => 'building-factory'], // Productos fabricados
        ['kind' => 'route', 'target' => 'tenant.mill_production.index', 'icon' => 'package-import'], // Ingreso de insumos
        ['kind' => 'route', 'target' => 'tenant.machine_type_production.index', 'icon' => 'settings-automation'], // Tipos de maquinaria
        ['kind' => 'route', 'target' => 'tenant.machine_production.index', 'icon' => 'engine'], // Maquinaria
        ['kind' => 'route', 'target' => 'tenant.packaging.index', 'icon' => 'box'], // Zona de embalaje
        ['kind' => 'route', 'target' => 'tenant.workers.index', 'icon' => 'users-group'], // Empleados
    ];

    $sidebarFavoriteIcons = [];
    foreach ($sidebarFavoriteIconEntries as $sidebarFavoriteIconEntry) {
        try {
            if ($sidebarFavoriteIconEntry['kind'] === 'route') {
                if (! \Illuminate\Support\Facades\Route::has($sidebarFavoriteIconEntry['target'])) {
                    continue;
                }
                $sidebarFavoriteIconUrl = route($sidebarFavoriteIconEntry['target'], $sidebarFavoriteIconEntry['params'] ?? []);
            } else {
                $sidebarFavoriteIconUrl = url($sidebarFavoriteIconEntry['target']);
            }
        } catch (\Throwable $sidebarFavoriteIconError) {
            continue;
        }

        $sidebarFavoriteIconParts = parse_url($sidebarFavoriteIconUrl);
        $sidebarFavoriteIconKey = 'route:' . rtrim($sidebarFavoriteIconParts['path'] ?? '', '/');
        if (! empty($sidebarFavoriteIconParts['query'])) {
            $sidebarFavoriteIconKey .= '?' . $sidebarFavoriteIconParts['query'];
        }

        $sidebarFavoriteIconSvg = $sidebarFavoriteIconMarkup[$sidebarFavoriteIconEntry['icon']] ?? null;
        if ($sidebarFavoriteIconSvg) {
            $sidebarFavoriteIcons[$sidebarFavoriteIconKey] = $sidebarFavoriteIconSvg;
        }
    }
@endphp

<script>
    window.sidebarFavoriteIcons = @json($sidebarFavoriteIcons);
    window.sidebarFavoriteIconFallback = @json($sidebarFavoriteIconMarkup['point']);
</script>
